...
Version for non-metric TSP with colors.
...
UI_ROOT = os.path.join(os.path.dirname(__file__), 'batch_ui_color')
...
def _get_tour(id_: int):
    ...
    if len(tour_cities) != len(tour_times):
        print(f'Warning: {len(tour_cities)} cities recorded, but {len(tour_times)} times recorded!')
...
def _send_cities(id_: int):
    ...
    return [json.dumps({
        ...
        'colors': batch[id_].colors.tolist(),
        'height': batch[id_].h,
        'width': batch[id_].w
    })]
