...
class DSNode:
    ...
    def __init__(self, value: Any, children: List[DSNode] = None, height: Number = 0):
        """
        Args:
...
class DSTree:
    ...
    def union(self, x: Hashable, y: Hashable, height: Number = None) -> DSNode:
        ...
        if x_parent is y_parent:
            return x_parent
        self._unions += 1
        new_parent = DSNode(None, [x_parent, y_parent], self._unions if height is None else height)
...
def cluster_boruvka(nodes: NDArray) -> Tuple[Set, DSNode]:
    ...
    while tree.sets > 1:
        ...
        for edge_len, edge in sorted(minimum_edges.values(), key=lambda t: t[0]):
            c = tree.union(*edge)#, edge_len)
            c.value = _calculate_centroid(c, nodes)
            result.add(edge)
    return result, tree.root
